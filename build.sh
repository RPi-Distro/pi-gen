...
EOF
		...
		if [ -f "${i}-packages-nr" ]; then
			...
			if [ -n "$PACKAGES" ]; then
				on_chroot << EOF
retry apt-get -o APT::Acquire::Retries=5 install --no-install-recommends -y $PACKAGES
EOF
				if [ "${USE_QCOW2}" = "1" ]; then
					on_chroot << EOF
apt-get clean
EOF
		...
		if [ -f "${i}-packages" ]; then
			...
			if [ -n "$PACKAGES" ]; then
				on_chroot << EOF
retry apt-get -o APT::Acquire::Retries=5 install -y $PACKAGES
EOF
				if [ "${USE_QCOW2}" = "1" ]; then
					on_chroot << EOF
apt-get clean
...
export DEPLOY_FILES="{DEPLOY_FILES:0}"
